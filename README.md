...
- 절차 지향 프로그래밍은 프로그램의 흐름을 순차적으로 처리하는 방식 -> 어떻게? 중심
- 객체 지향 프로그래밍은 실제 세계의 사물이나 사건을 객체로 보고, 이러한 객체들 간의 상호작용을 중심으로 프로그래밍하는 방식 -> 무엇을? 중심

절차 지향 프로그래밍은 클래스와 메서드를 통해 모듈화를 하여 깔끔하게 코드를 작성했지만 절차 지향 프로그래밍의 한계는 데이터와 기능이 
분리되어 있게 되어 유지보수 할 부분이 두 곳이 되는 것이다. (클래스에 있는 멤버변수가 변하면 각각의 메서드도 다 수정해야됨)
